...
export default class CommandLine {
    ...
    static async chooseDepartment(departments) {
    ...
    static async chooseEmployee(employees) {
    ...
    static async chooseManager(managers) {
        // Code to select a specific Employee, and return its id attribute.
        const managerId = await inquirer
            .prompt([
            {
                ...
                choices: (await managers).map((manager) => {
                    return {
    ...
    static async chooseAction() {
        inquirer
            ...
            if (answers.payrollAction === "View all departments") {
                ...
                const newDept = readlineSync.question("Please enter the name of the department you would like to add: \n");
                console.log(newDept);
                ...
                const roleTitle = readlineSync.question("Please enter the name of the role you would like to add: \n");
                const roleSalary = parseInt(readlineSync.question("Please enter the salaray for this role: \n"));
                ...
                const department_id = await CommandLine.chooseDepartment(Queries.getAllDepartments());
                // I need to do the same thing with the manager_id, but I need to pull the employee_id from the employees table.
                const manager_id = await CommandLine.chooseManager(Queries.getAllEmployees());
                const newRole = new Role(roleTitle, roleSalary, department_id, manager_id);
                ...
                const empFirstName = readlineSync.question("Please enter the new employee's first name: ");
                const empLastName = readlineSync.question("Please enter the new employee's last name: ");
                ...
                const empRoleId = await CommandLine.chooseRole(Queries.getAllRoles());
                ...
                const empManagerId = await CommandLine.chooseManager(Queries.getAllEmployees());
                console.log(`Manager ID = ${empManagerId}`);
                // Creating the Employee object.
                const newEmployee = new Employee(empFirstName, empLastName, empRoleId, empManagerId);
                // Debugging
                ...
                const empToUpdate = await CommandLine.chooseEmployee(Queries.getAllEmployees());
                ...
                // Select an employee from a list. Return the employee_ID of the selected entry.
                const employeeId = await CommandLine.chooseEmployee(Queries.getAllEmployees());
